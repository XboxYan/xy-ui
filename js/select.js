...
/*$('.ui_select_li').on('click', function (i, el) {
    var index = $(this).index();
    //$('#se').selectedIndex(3).trigger('change');
    $(this.parentNode.parentNode).removeClass('active');
    $(this.parentNode.parentNode.children[0]).selectedIndex(index).trigger('change');
    //$(this).addClass('selected').siblings().removeClass('selected');//1取一即可***
    //$(this.parentNode).next().children().eq(0).html(el.innerHTML);//2取一即可***
})*/
