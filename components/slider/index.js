...
export default class XySlider extends Base {
	static get observedAttributes() {
		return ["min", "max", "step", "disabled", "tips"];
	}
  ...
  renderTip(value) {
    ...
    if (this.vertical) {
      this.tipsEl.offset = '0,' + parseInt((.5 - percent) * (this.offsetHeight - 10));
    } else {
      this.tipsEl.offset = parseInt((percent - 0.5) * (this.offsetWidth - 10)) + ',0';
    }
  }
   ...
   if (name == "disabled") {
			this.slider.toggleAttribute("disabled", newValue !== null);
		} else {
